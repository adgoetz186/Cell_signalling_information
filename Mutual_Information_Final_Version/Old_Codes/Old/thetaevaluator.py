        ...
        for inputscale in range(9):
            ...
            for i in range(len(sumnp)):
                ...
                for x in range(4000):
                    ...
                    logpxgut1 = x*np.log(np.multiply(thetalist,u1list))-np.multiply(thetalist,u1list)-scipy.special.loggamma(x+1)
                    denomlog = np.average(pxgut1)
                    logdenomlog = (scipy.special.logsumexp(logpxgut1)-np.log(mcacc))
                    ...
                    logpxgut2 = x*np.log(np.multiply(thetalist,u2list))-np.multiply(thetalist,u2list)-scipy.special.loggamma(x+1)
