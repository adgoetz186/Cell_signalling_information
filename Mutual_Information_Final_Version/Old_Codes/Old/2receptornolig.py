...
def trial_receptor(x):
    ...
    kmin = 0.01
    kstep = 0.5
    Kalist = [kmin*10**(i*kstep) for i in range(17)]
    Kblist = [kmin*10**(i*kstep) for i in range(11)]
    totalrunsneeded= len(Kalist)*len(Kblist)
    runcount = 0
    timestart = time.time()
    for kap in Kalist:
        for kbp in Kblist:
            ...
            for Lv in Llist:
                ...
                class mRNA_Synth(Model):
                  def __init__(self, parameter_values = None):
                    ...
                    # Define parameters.
                    ka = Parameter(name = 'ka', expression = kap*Lv)
                    ...
                    self.add_species([R,LR,LRs])

                    # Define reactions.
                    LandR_LR = Reaction(name = "LandR_LR", reactants = {R:1}, products = {LR:1},
                                  propensity_function = "R*ka")
                    LR_LandR = Reaction(name="LR_LandR", reactants={LR: 1}, products={R: 1},
                                        propensity_function="LR*kb")
                    LR_LRs = Reaction(name="LR_LRs", reactants={LR: 1}, products={LRs: 1},
                                        propensity_function="LR*ka2")
                    LRs_LR = Reaction(name="LRs_LR", reactants={LRs: 1}, products={LR: 1},
