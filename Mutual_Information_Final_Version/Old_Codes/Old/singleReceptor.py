...
def trial_receptor(x):
    ...
    kd = 10**(-3)
    step = 0.5
    Kalist = [10**(step*i) for i in range(16)]
    totalrunsneeded= len(Kalist)
    runcount = 0
    timestart = time.time()
    for kap in Kalist:
        Lmin = 100
        ...
        for Lv in Llist:
            class mRNA_Synth(Model):
              def __init__(self, parameter_values = None):
                ...
                kd = Parameter(name='kb', expression=10 ** (-3))
                self.add_parameter([ka,kd])
                ...
                self.timespan(numpy.linspace(0, 5*1/kd,8))
