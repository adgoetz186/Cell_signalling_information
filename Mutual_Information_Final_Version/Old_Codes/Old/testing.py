...
timeStart = time.time()
d_results = EGFR_model.run(solver = gillespy2.solvers.TauLeapingSolver, t=10000, number_of_trajectories=1, seed=None, debug=False, show_labels=True)
print("Time Tau = " + str(time.time() - timeStart))
timeStart = time.time()
d_results = EGFR_model.run(solver = gillespy2.solvers.NumPySSASolver, t=10000, number_of_trajectories=1, seed=None, debug=False, show_labels=True)
print("Time SSA = " + str(time.time() - timeStart))
input()
print(d_results)
plt.plot(d_results[0]['time'], d_results[0]['sLR2'], '-r', label='U')
plt.title('Stochastic Switch')
plt.legend(loc = 'best')
plt.show()


d_results = EGFR_model.run(solver = BasicODESolver,show_labels = False)

plt.plot(d_results[0]['time'], d_results[0]['C'], '-r', label='U')
plt.plot(d_results[0]['time'], d_results[0]['sLR2'], '-b', label='V')
plt.title('Stochastic Switch')
plt.legend(loc = 'best')
plt.show()